...
--
-- ⚠ 维护警告：本文件是 schema_version=4 时 agents 表的冻结快照（"schema snapshot"）。
--   未来加 agents 列时，schema.sql 之外，"最新一份 migration"里的 agents_new DDL
--   也必须同步更新（runner 不知道自动 diff；漏改会导致 5→6 migration 的
--   SELECT * FROM agents 失败）。
BEGIN TRANSACTION;
